...
| P2 核心用户体验 | 25% | 44% | 进行中 | 4/9 项完成；会话管理、草稿和文本发送已在 Android 7.1.1 真机验证 |
...
| **总计** | **100%** | **46%** | **进行中** | P0/P1 已完成，P2 已完成 4/9 个任务 |
...
- SQLite schema 升级到 v2，`conversations.draft_text` 通过逐版本迁移加入；真实 v1 数据库升级测试确认 4 个种子会话与已有数据保留。
- 会话仓储已实现置顶、免打扰、标为已读、草稿保存和删除；删除由外键级联清理会话成员、消息与附件。
- 会话列表增加操作菜单和草稿摘要，聊天输入器支持 400 ms 防抖保存、重开恢复，发送事务同时清空草稿。
- 数据库测试 7/7、完整 Flutter 测试 10/10 通过；离线模块静态分析无问题。
- `0.4.0+5` Release 最终增量构建以退出码 0 完成，耗时 35 秒；AAPT 确认无 `INTERNET` 权限，APK v2 签名有效。
- 候选产物：`dist/android/TUIKit-OfflineDemo-P2-conversations-0.4.0+5.apk`，101,617,851 字节，SHA-256 `1BDB5EC997629B599637FD446DCA099A72FA1873528D1D0BF3E1AC8667785480`。

### 延期验收

- 用户当前取走 Redmi K80 Pro，按要求暂停所有 ADB 和真机操作；不会在手机重新可用前尝试安装或启动。
- P2-002、P2-004 继续保持未完成，等待真机验证置顶、免打扰、删除、草稿、文本发送和强制停止后的持久化。

### 工具链观察

- 本轮首次 `flutter build apk` 已生成可校验 APK，但命令收尾超过 5 分钟上限；随后 Gradle 增量构建和最终 Flutter 增量构建均以退出码 0 完成。若再次出现，应检查 Flutter/Gradle 收尾进程而不是重复等待。
- 可选代理仍只存在于被 Git 忽略的 `.local/network.ps1`；最终增量构建从 `application` 目录显式加载，代理地址未写入仓库。

## 2026-08-02

### P2 专用真机已验证

- 新测试设备为 360 `1707-A01`，Android 7.1.1 / API 25，无线 ADB 地址 `192.168.2.50:5555`；同时存在的 `192.168.2.30:5555` 保持离线且未操作。
- `0.4.0+5` 安装成功，系统报告 minSdk 24、targetSdk 36；首次冷启动 `Status: ok`、`TotalTime: 723 ms`，进程和前台 Activity 正常，AndroidRuntime 无崩溃。
- 搜索框通过屏幕键盘输入 `16` 后，从 4 个会话准确过滤出摘要含 `16:00` 的“全员公告”。
- 产品项目组通过菜单完成标为已读、置顶和免打扰；强制停止并重启后仍位于首位、未读角标消失，菜单保持“取消置顶”和“开启提醒”。
- 聊天输入 `draft` 后，列表显示 `[草稿] draft`；强制停止并重启后列表摘要和聊天输入框均恢复该草稿。
- 发送 `draft` 后输入框清空、消息气泡出现、会话摘要更新；再次强制停止并重启后消息仍存在，草稿保持为空。
- 删除离线助手时确认框显示“将删除离线助手及其本地消息和附件”；确认后会话数由 4 变为 3，强制停止并重启后仍为 3。
- `P2-002` 与 `P2-004` 验收完成。设备当前保留上述测试状态，便于人工复查。

### 环境观察

- Android 7.1.1 上 `adb shell input text` 无法可靠写入 Flutter TextField，测试改用系统屏幕键盘坐标输入；这属于 ADB 输入注入限制，真实键盘输入、草稿和发送均正常。
- 该设备的 UI Automator 层级导出偶有较长等待，但应用进程、前台 Activity 和最终层级结果均正常。
